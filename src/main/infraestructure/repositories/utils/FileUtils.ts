...
import { FileFilters } from "../../../application/dto/FileFilters";
...
export function listarArchivos(
  { dir,
    recursive,
    includeCreationDate,
    includeLastModDate }: FileFilters
): File[] {
  let resultados: File[] = [];
  ...
  for (const elemento of elementos) {
    ...
    if (elemento.isDirectory() && recursive) {
      resultados = resultados.concat(
        listarArchivos({ dir: rutaCompleta, recursive, includeCreationDate, includeLastModDate })
      );
